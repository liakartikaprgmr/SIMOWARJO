...
@section('content')
    <div class="container mx-auto max-w-4xl mt-6 px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-800 tracking-tight">Edit Jadwal Shift</h1>
                <p class="text-sm text-gray-500 mt-1">Perbarui jadwal shift karyawan yang sudah ada.</p>
            </div>
            <a href="{{ route('admin.penjadwalan.index') }}"
                class="flex items-center justify-center w-10 h-10 bg-white border border-gray-200 text-gray-600 rounded-full shadow-sm hover:bg-gray-50 hover:text-red-600 hover:border-red-200 transition-all focus:outline-none focus:ring-2 focus:ring-red-500" title="Kembali">
        ...
        <div class="bg-white p-6 md:p-10 rounded-2xl shadow-xl border border-gray-100/50 relative overflow-hidden">
            ...
            <form action="{{ route('admin.penjadwalan.update_bulk') }}" method="POST" class="relative z-10">
            @csrf
            <input type="hidden" name="ids_string" value="{{ $jadwalData->ids_string }}">
            
            <!-- Karyawan Input (Datalist for searchable) -->
            <div class="mb-6">
                <label for="karyawan_input" class="block text-sm font-semibold text-gray-700 mb-2">Pilih Karyawan</label>
                <div class="relative">
                    ...
                    <input list="karyawans" id="karyawan_input" value="{{ old('karyawan_input', $jadwalData->karyawan ? $jadwalData->karyawan->nama : '') }}" 
                ...
                <input type="hidden" name="id_karyawan" id="id_karyawan" value="{{ old('id_karyawan', $jadwalData->id_karyawan) }}">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="shift" id="shift" 
                        ...
                        required>
                        <option value="Pagi" {{ (old('shift') ?? $jadwalData->shift) == 'Pagi' ? 'selected' : '' }}>Pagi (08:00 - 16:00)</option>
                        <option value="Malam" {{ (old('shift') ?? $jadwalData->shift) == 'Malam' ? 'selected' : '' }}>Malam (20:00 - 04:00)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', $jadwalData->tanggal_mulai) }}" 
                            class="w-full px-3 py-2.5 bg-gray-50 border border-gray-300 rounded-xl focus:bg-white focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all duration-200" required>
                        <span class="text-gray-400 font-medium px-1">-</span>
                        <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', $jadwalData->tanggal_selesai) }}" 
                            class="w-full px-3 py-2.5 bg-gray-50 border border-gray-300 rounded-xl focus:bg-white focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all duration-200" required>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                ...
                <div>
                    ...
                    <input type="time" name="jam_masuk" id="jam_masuk" value="{{ old('jam_masuk', \Carbon\Carbon::parse($jadwalData->jam_masuk)->format('H:i')) }}" 
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl focus:bg-white focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all duration-200" required>
                </div>

                <!-- Jam Pulang -->
                <div>
                    <label for="jam_pulang" class="block text-sm font-semibold text-gray-700 mb-2">Jam Pulang</label>
                    <input type="time" name="jam_pulang" id="jam_pulang" value="{{ old('jam_pulang', \Carbon\Carbon::parse($jadwalData->jam_pulang)->format('H:i')) }}" 
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl focus:bg-white focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all duration-200" required>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                <a href="{{ route('admin.penjadwalan.index') }}" 
                    class="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-xl font-medium hover:bg-gray-50 hover:text-gray-900 transition-colors focus:ring-2 focus:ring-gray-200">
                    Batal
                </a>
                <button type="submit" 
                    class="flex items-center px-6 py-2.5 bg-blue-600 text-white rounded-xl font-medium hover:bg-blue-700 shadow-sm hover:shadow transition-all focus:ring-2 focus:ring-blue-500/50">
...
<script>
    ...
    // Auto-fill jam berdasarkan pilihan shift saat diedit manual (Opsional)

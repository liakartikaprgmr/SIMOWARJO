...
<div class="mt-16">
    ...
    {{-- HEADER --}}
    <div class="bg-orange-600 text-white rounded-xl p-6 mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold">Barang Keluar</h1>
            <p class="text-orange-200 text-sm mt-1">Catat pengeluaran barang dengan detail penerima</p>
        </div>
        <a href="{{ route('admin.kelola_barang.index') }}" class="bg-white text-orange-600 font-semibold px-4 py-2 rounded-lg hover:bg-orange-50 text-sm">
            ← Kembali ke Stok
        </a>
    </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow p-6">
                <h2 class="font-bold text-gray-800 mb-4 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" class="text-orange-500"><path fill="currentColor" d="M19 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2m-7 14l-5-5h4V8h2v4h4z"/></svg>
                    Form Barang Keluar
                </h2>
                <form action="{{ route('admin.kelola_barang.keluar.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="penerima" value="{{ old('penerima') }}" required
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
                            placeholder="Nama penerima / departemen">
                    </div>
                    <div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow overflow-hidden">
                <div class="p-4 border-b flex items-center justify-between flex-wrap gap-3">
                    <h2 class="font-bold text-gray-800">Riwayat Barang Keluar</h2>
                    <form method="GET" class="flex flex-wrap gap-2 items-end">
                        ...
                        <div>
                            <label class="block text-xs text-gray-500 mb-1">Penerima</label>
                            <input type="text" name="penerima" value="{{ request('penerima') }}" placeholder="Cari..."
                        ...
                        <a href="{{ route('admin.kelola_barang.keluar') }}" class="bg-gray-200 text-gray-700 px-3 py-1 rounded text-xs">Reset</a>
                    </form>
                </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-center font-bold text-orange-600">-{{ number_format($r->jumlah) }} {{ $r->barang->satuan ?? '' }}</td>
                                <td class="px-4 py-3 text-gray-600">{{ $r->penerima }}</td>
                                <td class="px-4 py-3 text-gray-500 text-xs">{{ $r->keperluan }}</td>
                                <td class="px-4 py-3 text-gray-500 text-xs">{{ $r->karyawan->nama ?? '-' }}</td>
                                <td class="px-4 py-3 text-center">
                                    <form action="{{ route('admin.kelola_barang.keluar.destroy', $r) }}" method="POST"
                            ...
                            @empty
                            <tr><td colspan="7" class="px-4 py-10 text-center text-gray-400">Belum ada data.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
